...
void QOpenGLWidgetOverride::paintGL() {
  ...
  double width = 2.f / static_cast<float>(columns + 1), height = 2.f / static_cast<float>(rows + 1);
  glVertex2f(static_cast<GLfloat>(width / 2.f), static_cast<GLfloat>(height / 2.f));
  glVertex2f(static_cast<GLfloat>(width * (columns + 0.5)), static_cast<GLfloat>(height / 2.f));
  glVertex2f(static_cast<GLfloat>(width / 2.f), static_cast<GLfloat>(height / 2.f));
  glVertex2f(static_cast<GLfloat>(width / 2.f), static_cast<GLfloat>(height * (rows + 0.5)));
  for (int row = 0; row < rows; ++row) {
    for (int column = 0; column < columns; ++column) {
      auto wall =
          static_cast<s21::Walls>(maze_model.GetMatrix()(row, column));
      if (wall == s21::kBottomWall || wall == s21::kBothWalls) {
        glVertex2f(static_cast<GLfloat>(width * (column + 1.5)), static_cast<GLfloat>(height * (row + 1.5)));
        glVertex2f(static_cast<GLfloat>(width * (column + 0.5)), static_cast<GLfloat>(height * (row + 1.5)));
      }
      if (wall == s21::kRightWall || wall == s21::kBothWalls) {
        glVertex2f(static_cast<GLfloat>(width * (column + 1.5)), static_cast<GLfloat>(height * (row + 1.5)));
        glVertex2f(static_cast<GLfloat>(width * (column + 1.5)), static_cast<GLfloat>(height * (row + 0.5)));
      }
    }
  }
  glEnd();
